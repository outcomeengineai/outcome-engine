...
function parseDate(raw: string | undefined): string | null {
  ...
  if (t < Date.UTC(2000, 0, 1) || t > Date.now() + 2 * 86_400_000) return null;
...
export function parseFeed(xml: string): FeedItem[] {
  ...
  while ((m = blockRe.exec(xml)) !== null) {
    ...
    const publishedAt = parseDate(
      tagContent(block, 'pubDate') ?? tagContent(block, 'published') ??
...
export function marketTerms(id: string, question: string, category: string): MarketTerms {
 ...
 */
export function matchesTerms(tokens: Set<string>, t: MarketTerms): boolean {
